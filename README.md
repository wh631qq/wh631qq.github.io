...
1. 用文本编辑器打开根目录的 `写文章.txt`，**只写正文**即可（标题固定为「随思」）。
2. 正文直接写字即可，换行规则：
   - **回车一次** = 接着写（不分行）
   - **空一行** = 新段落
3. 双击根目录的 `发布.bat`，它会**生成一篇新文章**（带时间戳的文件名）、更新首页列表、提交并推送到 GitHub。

一两分钟后网站就更新了。

> 每次发布会**新增一篇**，不会覆盖旧文；首页按标题分类、列出所有文章。
> 如果正文和最新一篇**相同**，会跳过、不重复发布。
> 要修改已发布的文章，直接编辑 `posts/` 目录下对应的 html 文件。

## 提交其他改动

如果只是改了「关于」页、样式、友情链接等**非文章**内容，双击根目录的 `其他改动.bat`，它会直接提交并推送到 GitHub（不会生成文章）。

如果你在 `posts/` 目录里**删除了某篇文章文件**，它还会自动从首页移除对应的失效链接，再一起提交。
